...
let cachedOidcDeps = null;

async function getOidcDeps() {
  if (cachedOidcDeps) {
    return cachedOidcDeps;
  }
  ...
  try {
    ...
    const oidcClient = oidcModule.default || oidcModule;
    const Strategy =
      passportModule.Strategy ||
      (passportModule.default && passportModule.default.Strategy) ||
      passportModule.default;

    if (!oidcClient || !Strategy) {
      throw new Error("OIDC dependencies loaded without required exports.");
    }

    cachedOidcDeps = { oidcClient, Strategy };
    return cachedOidcDeps;
  } catch (err) {
    console.error("[auth] Failed to load OIDC dependencies:", err.message);
    return null;
  }
}
...
const getOidcConfig = memoize(
  async () => {
    const deps = await getOidcDeps();
    if (!deps) {
      throw new Error("OIDC dependencies unavailable.");
    }
...
function isReplitAuthConfigured() {
  return Boolean(
    process.env.REPL_ID &&
...
async function setupAuth(app) {
  if (!isReplitAuthConfigured()) {
    console.warn("[auth] Replit auth is disabled because REPL_ID, SESSION_SECRET, or DATABASE_URL is missing.");
    return;
  }

  const deps = await getOidcDeps();
  if (!deps) {
    console.warn("[auth] Replit auth is disabled because OIDC dependencies are unavailable.");
    return;
  }

  const { Strategy, oidcClient } = deps;
