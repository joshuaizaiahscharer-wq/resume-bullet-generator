...
function isPurchasePaid(financialStatus) {
  const normalized = String(financialStatus || "").toLowerCase().trim();
  return (
    normalized === "paid" ||
...
function normalizeIncomingToken(value) {
  const raw = String(value || "").trim();

  // Tokens we generate are 64-char hex strings. Extract one if the value
  // includes accidental wrappers/punctuation from copy/paste.
  const hexMatch = raw.match(/[a-f0-9]{64}/i);
  if (hexMatch) {
    return hexMatch[0].toLowerCase();
  }

  // Fallback to raw value if it does not match the expected shape.
  return raw;
}
...
async function validateTemplateAccessToken(token) {
  const cleanToken = normalizeIncomingToken(token);
  if (!cleanToken) {
