...
function getBacklinks(currentSlug, allPosts) {
  return allPosts.filter(
    (p) =>
      p.slug !== currentSlug &&
      Array.isArray(p.internalLinks) &&
      p.internalLinks.includes(currentSlug)
  );
}
...
function renderBacklinksSection(backlinks) {
  ...
  const items = backlinks
    .map(
      (p) =>
        `<li class="blog-backlinks-item"><a href="/blog/${escapeHtml(p.slug)}" class="blog-backlinks-link">${escapeHtml(p.title)}</a></li>`
    )
    .join("\n");
  return `
    <section class="blog-backlinks" aria-label="Posts that reference this article">
      <h3 class="blog-backlinks-heading">Referenced In</h3>
      <ul class="blog-backlinks-list">${items}</ul>
    </section>
  `;
}
...
function renderBlogPostPage(post, siteUrl) {
  ...
  const backlinks = getBacklinks(post.slug, blogPosts);
