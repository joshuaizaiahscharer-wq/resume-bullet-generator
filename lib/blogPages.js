...
function getFallbackPostImage(post) {
  const seed = slugifyHeading(
    [post.imagePrompt, post.title, post.category, "career", "resume"]
      .filter(Boolean)
      .join("-")
  ) || "career-resume";
...
function getPostImage(post) {
  const explicit = String(post.image || "").trim();
  if (explicit && !explicit.includes("source.unsplash.com")) return explicit;
  return getFallbackPostImage(post);
}
...
function renderBlogCard(post) {
  ...
  const imageMarkup = `<img class="blog-card-image" src="${escapeHtml(imageUrl)}" alt="${escapeHtml(post.title)}" loading="lazy" />`;
...
function renderFeaturedPost(post) {
  const readTime = estimateReadingTime(post);
  const imageUrl = getPostImage(post);
  const imageMarkup = `<img class="featured-image" src="${escapeHtml(imageUrl)}" alt="${escapeHtml(post.title)}" loading="lazy" />`;
  return `
...
function renderPostContent(post) {
  if (!Array.isArray(post.sections) || !post.sections.length) {
    const normalized = normalizeGeneratedBlogContent(post.content || "");
    if (isLikelyHtmlContent(normalized)) {
      return `<section class="blog-section">${sanitizeBlogHtml(normalized)}</section>`;
    }
...
function isLikelyHtmlContent(content) {
  return /<\s*\/?\s*(h2|h3|p|ul|li|a|strong|em|b|i)\b/i.test(String(content || ""));
}

function sanitizeBlogHtml(content) {
  const allowedTags = new Set(["h2", "h3", "p", "ul", "li", "a", "strong", "em", "b", "i"]);
  const stripped = String(content || "")
    .replace(/<\s*(script|style|iframe|object|embed)[^>]*>[\s\S]*?<\s*\/\s*\1\s*>/gi, "");

  return stripped.replace(/<\/?([a-z0-9-]+)([^>]*)>/gi, (fullTag, rawTag, rawAttrs) => {
    const tag = String(rawTag || "").toLowerCase();
    if (!allowedTags.has(tag)) return "";

    if (String(fullTag).startsWith("</")) {
      return `</${tag}>`;
    }
  ...
  <body class="blog-body blog-body--post">
    ...
    <div class="blog-post-layout">
      <main class="blog-article-main">
        <section class="blog-post-cta" aria-label="Primary call to action">
          <h2>Improve your resume instantly with AI</h2>
          <p>Use BulletAI to turn weak experience lines into stronger, interview-ready bullet points.</p>
          <a href="/resume-template-builder" class="blog-post-cta-btn">Try the tool</a>
        </section>
        ...
        <article class="blog-article" aria-labelledby="post-title">
          ...
          <img class="blog-article-image" src="${escapeHtml(postImage)}" alt="${escapeHtml(post.title)}" loading="lazy" />
