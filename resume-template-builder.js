...
const elementRefs = {
  ...
  clearBtn: document.getElementById("clearResumeBtn"),
  saveIndicator: document.getElementById("resumeSaveIndicator"),
...
let localResumeLoaded = false;

function getLocalResumeKey(userId) {
  return userId ? `${LOCAL_RESUME_KEY_PREFIX}_${userId}` : `${LOCAL_RESUME_KEY_PREFIX}_anonymous`;
}

function getCurrentUserId() {
  return resumeBuilderState.signedInEmail || null;
}

function clearAllLocalResumes() {
  Object.keys(localStorage).forEach((key) => {
    if (key.startsWith(LOCAL_RESUME_KEY_PREFIX)) {
...
function subscribeToPaymentStatus(_userId) {
}
...
function buildLocalResumePayload() {
  return {
    savedAt: Date.now(),
    hasSubmitted: resumeBuilderState.hasSubmitted,
    isFormCollapsed: resumeBuilderState.isFormCollapsed,
...
function showSavedIndicator(message = "Saved ✓") {
  if (!elementRefs.saveIndicator) return;
  elementRefs.saveIndicator.textContent = message;
  elementRefs.saveIndicator.classList.remove("hidden");
  clearTimeout(saveIndicatorTimer);
  saveIndicatorTimer = setTimeout(() => {
    if (elementRefs.saveIndicator) {
      elementRefs.saveIndicator.classList.add("hidden");
    }
  }, 2000);
}

const saveResume = (data, userId) => {
  const key = getLocalResumeKey(userId !== undefined ? userId : getCurrentUserId());
  try {
    localStorage.setItem(key, JSON.stringify(data));
    console.log("Saved under:", key);
...
function clearSavedResume(userId) {
  const key = getLocalResumeKey(userId !== undefined ? userId : getCurrentUserId());
  try {
    localStorage.removeItem(key);
    console.log("Removed:", key);
...
function scheduleLocalAutoSave() {
  ...
  localAutoSaveTimer = setTimeout(() => {
    saveResume(buildLocalResumePayload(), getCurrentUserId());
  }, 1000);
}

function applyLoadedResume(saved) {
  if (!saved || typeof saved !== "object") return false;

  if (saved.formData && typeof saved.formData === "object") {
    resumeBuilderState.formData = saved.formData;
  }
  if (saved.generatedData && typeof saved.generatedData === "object") {
    resumeBuilderState.generatedData = saved.generatedData;
  }
  if (saved.resumeStyle) {
    resumeBuilderState.resumeStyle = saved.resumeStyle;
  }
  if (saved.resumeFont) {
    resumeBuilderState.resumeFont = saved.resumeFont;
  }
  ...
  resumeBuilderState.hasSubmitted = Boolean(saved.hasSubmitted);
  resumeBuilderState.isFormCollapsed = Boolean(saved.isFormCollapsed || saved.hasSubmitted);
  return true;
}
...
async function loadSavedResumeForSignedInUser() {
  ...
  resumeBuilderState.isCloudResumeLoading = true;
  updateAuthUi();
  ...
  try {
    ...
    // Mirror DB copy into user-specific localStorage key as backup.
    saveResume(saved, resumeBuilderState.signedInEmail);

    console.log("Resume loaded from DB");
  } catch (_err) {
    // Keep localStorage as fallback backup — load user-specific key.
    const backup = loadResume(resumeBuilderState.signedInEmail);
    if (backup) {
      applyLoadedResume(backup);
    }
  } finally {
    resumeBuilderState.isCloudResumeLoading = false;
    updateAuthUi();
  }
}
...
async function getSupabaseAccessToken() {
  try {
    const cfgResp = await fetch("/api/public-auth-config");
    if (!cfgResp.ok) return null;
    const cfg = await cfgResp.json();
    if (!cfg?.supabaseUrl) return null;

    const match = cfg.supabaseUrl.match(/https?:\/\/([^.]+)\.supabase\.co/);
  ...
  try {
    ...
    if (resumeBuilderState.signedInEmail) {
      ...
      if (session?.user?.email) {
        ...
        await loadSavedResumeForSignedInUser();
        try {
          await fetch("/api/resume-builder/sync-payment", {
            method: "POST",
            credentials: "include",
...
function updateAuthUi() {
  ...
  if (elementRefs.authStatus) {
    ...
    } else if (resumeBuilderState.isCloudResumeLoading) {
      elementRefs.authStatus.textContent = "Loading saved resume...";
    } else {
  ...
  if (elementRefs.saveBtn) {
    elementRefs.saveBtn.disabled = false;
  }

  if (elementRefs.clearBtn) {
    elementRefs.clearBtn.disabled = false;
  }
}
...
function ResumeBuilderForm() {
  ...
  function bindFormEvents() {
    ...
    form.addEventListener("submit", (event) => {
      ...
      saveResume(buildLocalResumePayload());
      refreshUi({ animatePreview: true });
      // Autosave after generating the resume so progress is never lost.
      saveCurrentResumeForSignedInUser().then((saved) => {
...
async function fetchAccessState(supabaseToken) {
  try {
    const headers = {};
    if (supabaseToken) headers["Authorization"] = `Bearer ${supabaseToken}`;
    const response = await fetch("/api/resume-builder/access", { credentials: "include", headers });
    if (!response.ok) {
      resumeBuilderState.hasPaid = false;
      resumeBuilderState.isUnlocked = false;
      return;
...
function bindGlobalEvents() {
  ...
  if (elementRefs.saveBtn) {
    elementRefs.saveBtn.addEventListener("click", async () => {
      ...
      if (resumeBuilderState.signedInEmail && resumeBuilderState.hasSubmitted) {
  ...
  if (elementRefs.clearBtn) {
    elementRefs.clearBtn.addEventListener("click", () => {
      clearSavedResume(getCurrentUserId());
      resetFormState();
      refreshUi();
    });
  }
  ...
  if (resetStorageBtn) {
    resetStorageBtn.addEventListener("click", () => {
      clearAllLocalResumes();
      resetFormState();
      showSavedIndicator("All saved resumes cleared");
      refreshUi();
    });
  }
...
async function handleRevise(key, fieldType, btn, form) {
  ...
  try {
    ...
    // Autosave after AI revision so the updated content is persisted.
    saveCurrentResumeForSignedInUser();
  } catch (err) {
    if (statusEl) {
...
function resetFormState() {
  ...
  resumeBuilderState.generatedData = null;
  resumeBuilderState.hasSubmitted = false;
  resumeBuilderState.isFormCollapsed = false;
}
...
async function init() {
  ...
  // Anonymous fallback — may be overridden once cloud auth resolves user identity.
  const localSavedResume = loadResume(null);
  if (localSavedResume && applyLoadedResume(localSavedResume)) {
    localResumeLoaded = true;
  }
